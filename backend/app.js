...
let db = new sqlite3.Database( path.resolve(__dirname, 'db.sqlite'), (err) => {
  if (err) {
    ...
    db.run(
      "CREATE TABLE IF NOT EXISTS meetups(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, url TEXT, address TEXT, description TEXT)"
    );
...
app.post("/meetups/", (req, res) => {
  db.serialize(() => {
    db.run(
      "INSERT INTO meetups(id,title,url,address,description) VALUES(?,?,?,?,?)",
      [
        req.body.id,
        req.body.title,
        req.body.url,
